...
if st.sidebar.button('Run Prediction'):
    ...
    root_url_backend = 'https://api-predict.onrender.com' #'http://127.0.0.1:8000' #'https://api-predict.onrender.com'
    
    # url des endpoints
    url_endpoint_predict = root_url_backend + '/predict'
    url_endpoint_interpret = root_url_backend + '/interpretability'
    
    
    response_predict = requests.post(url_endpoint_predict, json={'idx_client': idx_client})
    response_interpret = requests.post(url_endpoint_interpret, json={'idx_client': idx_client})
    ...
    st.write("Interpretation of the prediction: ")
    display_interpretability(response_interpret)
